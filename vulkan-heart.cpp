...
void CreateVKTextureImage() {
    ...
    mipLevels =
        (uint32_t)(std::floor(std::log2(std::max(image.width, image.height)))) +
        1;
    ...
    createImage(
        image.width, image.height, mipLevels, VK_SAMPLE_COUNT_1_BIT,
    ...
    //                       VK_IMAGE_LAYOUT_UNDEFINED,
    //                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
    generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, image.width,
                    image.height, mipLevels);
}
...
void processInput(GLFWwindow *window, Camera &camera) {
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        ...
        camera.up = normalize(
            cross(cross(camera.direction, camera.up), camera.direction));
    }
